...
type ETH2LDBatchReturn = [
  ...BaseBatchReturn,
  ReturnedENS['getExpiry'],
  ReturnedENS['getPrice'],
]

const useBasicName = (name?: string | null, normalised?: boolean) => {
  const ens = useEns()
  ...
  const { supported: supportedTLD } = useSupportsTLD(normalisedName)

  const { data: batchData, ...query } = useQuery(
  ...
  const { isLoading: batchLoading } = query
  ...
  const truncatedName = normalisedName
    ? truncateFormat(normalisedName)
    : undefined
