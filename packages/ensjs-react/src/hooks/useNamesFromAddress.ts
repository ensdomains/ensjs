...
const useNamesFromAddress = ({
  ...
  const { blockTimestamp, isLoading: isBlockTimestampLoading } =
    useBlockTimestamp()
  ...
  const mergedData = useMemo(() => {
    ...
    const nameMap = data.reduce((map, curr) => {
      ...
      if (newItem.registrationDate)
        newItem.registrationDate = new Date(newItem.registrationDate)
      return newMap
  ...
  const filterFunc = useMemo(() => {
    const baseFilter = (n: ReturnedName) => {
      if (
        n.expiryDate &&
        blockTimestamp &&
        n?.expiryDate.getTime() < blockTimestamp - GRACE_PERIOD
      )
        return false
    ...
    if (filter === 'registration')
      secondaryFilter = (n: ReturnedName) => !!n.isRegistrant
    if (filter === 'domain')
      secondaryFilter = (n: ReturnedName) => !!n.isController
    if (filter === 'wrappedDomain')
      secondaryFilter = (n: ReturnedName) => !!n.isWrappedOwner
    if (search)
      searchFilter = (n: ReturnedName) =>
        n.name.toLowerCase().indexOf(search.toLowerCase()) !== -1
    return (n: ReturnedName) =>
      baseFilter(n) && secondaryFilter(n) && searchFilter(n)
  ...
  const pages = useMemo(
    () =>
      ...
      (resultsPerPage === 'all'
        ? [sortedData]
        : chunkArr(sortedData, resultsPerPage)),
  ...
  return {
    ...query,
    currentPage,
    isLoading: query.isLoading || isBlockTimestampLoading,
    pageLength: pages?.length || 0,
    nameCount: sortedData?.length || 0,
  }
}

export default useNamesFromAddress
