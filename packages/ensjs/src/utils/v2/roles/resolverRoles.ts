 ...
 * never mix the two sets. A role is held either on the root resource (every
 * name on the resolver) or on the resource of a single setter argument; there
 * is no per-name scope, because a resolver is already per account.
 *
 * These bit positions are verified against the deployed implementation via
 * `decodeSetter`, which returns the role bitmap each setter requires.
 */

/** Nybble 0 — set address records. Root, or scoped to one coin type. */
export const RESOLVER_ROLE_SET_ADDRESS = 1n << 0n
/** Nybble 1 — set text records. Root, or scoped to one text key. */
...
/** Nybble 3 — set ABI records. Root, or scoped to one content type. */
export const RESOLVER_ROLE_SET_ABI = 1n << 12n
/** Nybble 4 — set interface implementer records. Root, or scoped to one interface id. */
...
/** Nybble 6 — set data records. Root, or scoped to one data key. */
export const RESOLVER_ROLE_SET_DATA = 1n << 24n
/** Nybble 7 — link names to records. Root only. Replaced `ROLE_SET_ALIAS`. */
...
/** Every resolver role and every admin role. */
...
/** Roles that can be narrowed to a single setter argument. */
