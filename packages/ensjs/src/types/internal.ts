 ...
 */
export function ASSERT_NO_TYPE_ERROR<T>(_: T): asserts _ is ExcludeTE<T> {}

/**
 * Type assertion helper that removes TypeErrors from a union type.
 * This is a unit function that returns it's input and only exists to help TypeScript's type system.
 */
export function EXCLUDE_TYPE_ERROR<T>(_: T): ExcludeTE<T> {
  return _ as ExcludeTE<T>
}
