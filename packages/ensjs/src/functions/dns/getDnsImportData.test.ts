...
describe('new', () => {
  it('returns all rrsets', async () => {
    ...
    if (result.isLegacy) throw new Error('Expected non-legacy result')
    expect(result.data.length).toBeGreaterThan(0)
    const decodedProofs = decodeProofs(result.data)
...
describe('legacy', () => {
  it('returns all rrsets when no proofs are known', async () => {
    const result = await getDnsImportData(publicClientWithLegacyDns, {
      name: 'taytems.xyz',
    })
    if (!result.isLegacy) throw new Error('Expected legacy result')
    expect(result.data.rrsets.length).toBeGreaterThan(0)
    expect(result.data.proof).toBeInstanceOf(Uint8Array)
    const decodedProofs = decodeProofs(result.data.rrsets)
    const rootProofs = decodedProofs.filter((x) => x.signature.name === '.')
    const tldProofs = decodedProofs.filter((x) => x.signature.name === 'xyz')
    const twoLDProofs = decodedProofs.filter(
      (x) => x.signature.name === 'taytems.xyz',
  ...
  it('returns rrsets up to the first unknown proof', async () => {
    const tx = await importDnsName(walletClientWithLegacyDns, {
      name: 'taytems.xyz',
      account: accounts[0],
      dnsImportData: await getDnsImportData(publicClientWithLegacyDns, {
        name: 'taytems.xyz',
      }),
    })
